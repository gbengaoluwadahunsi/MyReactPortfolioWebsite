    ...
    <header className="">
      <div className="grid grid-cols-2 px-16 p-10">
        ...
        <div ref={navigationRef} className={`grid grid-rows-7 fixed bg-slate-100 justify-self-end w-1/4 z-50 pr-6 py-6 toggle-section ${isMenuOpen ? 'show' : 'hide'}`}>
          ...
          <div className={`row-span-6 grid grid-rows-7 gap-2 color font-bold  navigation ${isMenuOpen ? 'menuShow' : 'menuHide'}`}>

           <div className=" row-span-1">
           <img src={Image}  className=" w-2/5 rounded-full  cursor-pointer background hover:border-rose-600 border-4"  alt="Gbenga Oluwadahunsi picture" />
           </div>
